...
# Manifest excludes itself and this one-time transport material.
old=json.loads((root/'RELEASE_MANIFEST.json').read_text(encoding='utf-8'))
entries=[]
for p in sorted(root.rglob('*')):
    if not p.is_file() or '.git' in p.parts or p.name=='RELEASE_MANIFEST.json' or p.as_posix() in {'tools/conservative-soi-pass.py','.github/workflows/conservative-soi-pass.yml'}:
        continue
...
if manifest['fileCount']!=176 or len(entries)!=175:raise RuntimeError('Unexpected manifest count')
