...
for e in entries:
    ...
    if not rel.startswith('public/'): raise RuntimeError(f'Non-public package file missing/mismatched after payload extraction: {rel}')
    url=PREVIEW+rel[len('public/'):]; p.parent.mkdir(parents=True,exist_ok=True); err=None
    for attempt in range(3):
        try:
            req=urllib.request.Request(url,headers={'User-Agent':'ProddyG-NorthEdge-Cutover/1.0'})
            with urllib.request.urlopen(req,timeout=30) as r: data=r.read()
            got=hashlib.sha256(data).hexdigest()
            if len(data)!=e['bytes'] or got!=e['sha256']: raise RuntimeError(f'Integrity mismatch for {rel}: bytes {len(data)}/{e["bytes"]}, sha {got}/{e["sha256"]}')
            p.write_bytes(data); print('verified download',rel); err=None; break
...
if problems: raise RuntimeError('Manifest verification failed: '+'; '.join(problems[:20]))
